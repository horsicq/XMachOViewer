...
if [ -z "$X_ERROR" ]; then
    ...
    if [ -z "$X_ERROR" ]; then
        ...
        cp -f $X_SOURCE_PATH/LINUX/xmachoviewer.desktop                    $X_SOURCE_PATH/release/$X_BUILD_NAME/usr/share/applications/
        ...
        cp -Rf $X_SOURCE_PATH/images/                                       $X_SOURCE_PATH/release/$X_BUILD_NAME/usr/lib/xmachoviewer/
